...
async def _rabbit(unzipbot, callback_query):
    ...
    try:
        ...
        stop = datetime.now()
        await msg.reply(
            f"🔺Extraction Done Successfully..! \n\nTook {round((stop - start).total_seconds() / 60, 2)} minutes \n\n🔸For more bots visit @Regaltos_botz")
    except rarfile.RarCannotExec:
        await msg.reply("**ERROR :** 🔺This File is corrupted. Cannot extract content. \n\n"
        ...
        await unzipbot.send_message(msg.chat.id, "**ERROR : **" + str(
            e) + "\n\nForward this message to @Regaltos_botz_support to solve this problem.", quote=True)
    finally:
